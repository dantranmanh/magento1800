...
//            'amlanding_cp.product_id = e.entity_id AND amlanding_cp.category_id IN (' . implode(',', $deepCategories) . ')'

            $join = Mage::getSingleton('core/resource')->getConnection('core_read')->quoteInto("amlanding_cp.product_id = e.entity_id AND amlanding_cp.category_id IN (?)", $deepCategories);

              $collection->getSelect()->joinInner(
                   array('amlanding_cp' => Mage::getResourceModel('catalog/product')->getTable('catalog/category_product')
                   ),
                  $join,
                   array()
               );
